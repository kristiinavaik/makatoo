...
Spontaanse kõne lindistamine ja üleskirjutamine on kahtlemata väga kasulik ning töö- ja ajamahukas protsess, mistõttu tekib seda suurem vajadus ühtse transkribeerimistava järele. Soovitusi, kuidas korpuse märgendamist ja standardiseerimist morfoloogilise analüüsi tarbeks paremaks muuta, on mitmeid. Esimene ettepanek puudutab vigaseid vorme. Soovitan kindlasti veainfot esitada [: ] kodeeringu abil (kooloni järele lisatakse sõna kirjakeelne vaste), sest loodud programm on ülesehitatud nii, et ainult seda tüüpi vea esitamisel analüüsitakse nurksulgude vahel olevat sisu. Rõhutan, et selle veakodeeringu kasutamisega peab kindlasti ettevaatlik olema. Teine ettepanek puudutab eelkõige onomatopoeetilisi sõnu ja häälitsusi. CHILDES-i konventsioonide järgi on iga sõna külge võimalik liita spetsiaalseid märgendusi: @\emph{o} on onomatopoeetilise sõna markeerimiseks, @\emph{l} abil saab ühte ja @\emph{k} abil mitut häälikut markeerida, @\emph{c} abil saab lapse väljamõeldud vormi markeerida, @\emph{z} abil saab transkribeerija kasutada oma defineeritud märgendust jpm. Selliste spetsiaalsete märgenduste kasutamine teeks näiteks onomatopoeetiliste sõnade ja häälitsuste tuvastamise lihtsamaks. Praegune olukord näeb ette, et kasutajasõnastikku
täiendatakse käsitsi, kuid spetsiaalsete märgenduste kasutamine võimaldaks teha seda
automaatselt.
...
Language and its production is one of the fundamental features of human species, however the question how language is acquired, has not yet been answered. The debate around language acquisition has risen attention to take different language theories and data collection methods even further. First studies on children's language development were based on diary notes, but the evolution of technology led the way in collecting large naturalistic datasets allowing systematically document and analyze child's and child directed speech. When computers became more available, transcripts were digitally archived which made the process of sharing the data publicly much more easier. But with that, the necessity for a standardized transcription system grew.

In 1984 Brian MacWhinney and Catherine Snow took the initiative to start a project called CHILDES database. The aim was to bring together child language corpora and make them publicly available for all researchers. Speech recordings have all been transcribed and coded in a uniform way using CHAT format. For assisting the researcher in transcribing, coding and analyzing the data a special software tool CLAN has been developed. CHILDES Estonian corpus consists of seven subcorpora: Argus, Beek, Kapanen, Kohler, Kõrgesaar, Vija and Zupping.

My initial plan was to build a morphologically annotated Estonian child language corpus from scratch, because the CLAN software isn't suitable for analyzing Estonian. Although all these subcorpora use the CHAT format for transcribing and coding the text, the discrepancy of the coding style among all corpora was too imminent to ignore. Therefore I had to take a step back by using previously converted XML files. For the linguistic annotation I had to implement a morphological annotation into these XML files. My new goal was to analyze the corpus on a basic level, asess its outcome and offer some recommendations on how to uniform the coding style for it to be much more suitable for the analyser.

Estonian child language corpus was analyzed without customizing the analyser. We have to bear in mind that child's and child directed speech differ from edited written language in many levels. Main goal was to assess the analyser's outcome and see how it could be customized for the purpose of analyzing child's and child directed speech.

Considering child directed speech properties one must assume that the analyzer's outcome wouldn't be very good, but on the controrary, the results show that the analyzed words in child directed speech varies between 94--98\%. Small \% of unknown words is very good, but it implicates that the child directed speech has been transcribed in manner that is similar to the written language. Analyzed words in child's speech varies between 57--96\%. This shows that the coding style among all subcorpora differs tremendously. Child's language is transcribed in a more speech-like manner, being rich with erroneous forms. One solution would be to follow each erroneous form with a standard form by using [: ] notation. It is necessary to notate erroneous forms because sometimes even with the context it is impossible to understand what the child actually meant by producing that particular word. With this notation there's a risk for transcribers to overthink it. Further, every transcriber has to think about what kind of erroneous forms to categorize. It would be inappropriate to categorize babbling, uttering sounds and monosyllabic words as erroneous, because the child's speech and knowledge of language is continuously evolving and these nonstandard forms give a little insight on how language is acquired.

Ideally, every linguistically annotated corpus should be disambiguated, but in morphologically rich languages some ambiguity is inevitable. While manually assessing the outcome of the analyzer it came clear that some word groups don't fit into the traditional part of speech distribution, e.g. onomatopoeic words, uttering sounds etc. There's a way of customizing an existing morphological analyser with a creation of a custom userdictionary. At the moment it is clear that proper names and reduplicative words need to be added to the userdictionary, but for a proper plan the words that were unknown or falsely analyzed by the analyser need to be looked at more closely.

Collecting spontaneous speech data is a labour intensive and time consuming process, therefore the necessity of having a standardized transcription habit is with an upmost importance. As I mentioned before one goal was to offer some recommendations on how to uniform the coding style for it to suitable for the analyser. Firstly, how to notate the erroneous forms. I strongly suggest to use the [: ] notation because the program was built to only analyze that erroneous form category, which is also used in CLAN software. Secondly, CHAT format offers special form markers that use ``@'' symbol with one or two additional letters, e.g. @\emph{o} is used for onomatopoeia, @\emph{l} is used for one letter, @\emph{k} is used for multiple letters, @\emph{c} is used for child-invented forms etc. These form markers are especially useful for labeling onomatopoeic words and babbling sounds. Currently it is difficult to detect these forms, it has to be done manually but with the use of these special form markers it would be possible to add them into the custom userdictionary in an automated way.
